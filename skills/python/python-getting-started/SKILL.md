...
| Install | `pip install "git+https://github.com/MuHamza30/paypal-server-sdk-python@main"` |
...
- `paypalserversdk/api_helper.py` — `APIHelper`, including the `SKIP` sentinel and the date-time wrappers.
- `paypalserversdk/logging/` — `LoggingConfiguration` and the request/response halves, when generated.

## Install

This SDK is published out of `https://github.com/MuHamza30/paypal-server-sdk-python`, branch `main` — take that branch explicitly rather than the repository default, which is not necessarily where this SDK is released from. Which registry that pipeline pushes to is a property of the pipeline rather than of the SDK, so confirm the feed before assuming the public one.
...
pip install "git+https://github.com/MuHamza30/paypal-server-sdk-python@main"
```
...
pip install -e ./path/to/paypal_serversdk

# From a built wheel, once `python -m build` has produced one in the SDK's `dist/`:
pip install ./path/to/paypal_serversdk/dist/paypal_serversdk-2.29.0-py3-none-any.whl
```
...
```bash
pip show paypal_serversdk        # installed version and its location
pip install --upgrade --force-reinstall paypal_serversdk==2.29.0
```
...
```bash
pip install --extra-index-url https://user:${PIP_TOKEN}@pypi.internal.example/simple paypal_serversdk
```
...
```bash
git clone --depth 1 --branch main https://github.com/MuHamza30/paypal-server-sdk-python
```
...
- `README.md` and `doc/` — a generated, human-readable index: `doc/client.md`, `doc/auth/*.md`,
  ...
  `doc/api-helper.md`, and `doc/api-response.md` when the SDK wraps responses. **Grep `doc/` first** — it is the fastest way to find an operation, its
