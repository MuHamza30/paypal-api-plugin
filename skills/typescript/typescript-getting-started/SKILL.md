...
| Install | `npm install "git+https://github.com/MuHamza30/paypal-server-sdk-typescript#main"` |
...
This SDK is published out of `https://github.com/MuHamza30/paypal-server-sdk-typescript`, branch `main` — take that branch explicitly rather than the repository default, which is not necessarily where this SDK is released from. Which registry that pipeline pushes to is a property of the pipeline rather than of the SDK, so confirm the feed before assuming the public one.

```bash
# Install this SDK:
npm install "git+https://github.com/MuHamza30/paypal-server-sdk-typescript#main"

# Or, from a packed tarball — `npm pack` inside the SDK directory writes that file:
npm pack
npm install ./paypal-server-sdklib-2.29.0.tgz
```
...
```bash
npm ls paypal-server-sdklib                            # installed version, and where it came from
npm install paypal-server-sdklib@2.29.0   # move to a regenerated build
```
...
```bash
git clone --depth 1 --branch main https://github.com/MuHamza30/paypal-server-sdk-typescript
```
