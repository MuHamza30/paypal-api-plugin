...
`_Unknown` is the one member emitted **without** an `[EnumMember]`, so it has no wire value:
`ApiHelper.JsonSerialize(value).Trim('"')` on it yields the C# member name. A `switch` over the
