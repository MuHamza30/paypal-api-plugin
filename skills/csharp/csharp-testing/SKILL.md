...
**Read the class before asserting on a payload field** — `e.Message` is the SDK's reason string, not
the API's `message`, and a colliding field is handled as **csharp-error-handling** describes for this
build. Test
