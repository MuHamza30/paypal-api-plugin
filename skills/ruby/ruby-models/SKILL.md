...
**Date-time** attributes are Ruby `DateTime` objects: `from_hash` parses them with `DateTimeHelper.from_*`
and the model carries a matching `to_custom_{attribute}` that re-formats them in the wire format the spec
declared — RFC 3339, RFC 1123 or a Unix timestamp. You pass and receive `DateTime`; the SDK handles the
string form.
