...
| Gem id | `paypal-server-sdk` (version `2.0.0`) — `s.name` / `s.version` in `paypal_server_sdk.gemspec` |
...
gem install paypal-server-sdk-2.0.0.gem
```
...
```bash
gem list paypal-server-sdk --details   # installed versions and the source of each
bundle update paypal-server-sdk        # outside Bundler, `gem install` the new version
```
...
```bash
git clone --depth 1 --branch main https://github.com/paypal/PayPal-Ruby-Server-SDK
```
...
> **`doc/` is in the SDK's repository, not in the installed gem.** A gem ships only what its gemspec's
> `files` list names, and the generated docs are not in it — so the gem root that
> `bundle show paypal-server-sdk` prints holds the library and little else. One `ls` of that root settles
> what your gem actually carries, before you plan a lookup around `doc/`.
>
> When you have the repository, grep `doc/` first — it is the fastest route to an operation, and its
> per-controller pages carry the error table. When you have only the gem,
> `lib/paypal_server_sdk/controllers/` is the equivalent starting point, and the operation's own
> signature replaces the copy-pasteable call.
