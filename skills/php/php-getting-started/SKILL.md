...
This SDK is published out of `https://github.com/MuHamza30/paypal-server-sdk-php`, branch `main` — take that branch explicitly rather than the repository default, which is not necessarily where this SDK is released from. Which registry that pipeline pushes to is a property of the pipeline rather than of the SDK, so confirm the feed before assuming the public one.
...
`libs/paypalserversdk` works and is the layout this guide assumes — and note that it must **not**
go under `vendor/`, which Composer manages itself:

```bash
# 1. resolve the SDK's own dependencies, inside the SDK directory
composer install --working-dir=libs/paypalserversdk --no-dev --no-interaction

# 2. from the consuming project root, register it and require it
composer config repositories.sdk-local '{"type":"path","url":"./libs/paypalserversdk","options":{"symlink":false}}'
composer require apimatic-sdks/paypalserversdk:@dev --no-interaction
```
...
```bash
composer show apimatic-sdks/paypalserversdk     # installed version and where it came from
composer update apimatic-sdks/paypalserversdk   # pick up a regenerated SDK
```
...
```bash
git clone --depth 1 --branch main https://github.com/MuHamza30/paypal-server-sdk-php
```
