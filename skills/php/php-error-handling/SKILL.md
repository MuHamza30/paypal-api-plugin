...
| --- | --- | --- |
| present — the **wrapper build** | returns the wrapper; `getResult()` is the **raw decoded body** | throws `ApiException` with a `getCode()` of `0` |
...
**raw decoded body** — a plain PHP array — and `->getMessage()` on it is a fatal "call to a member function
on array"; there, keep a `try/catch` for `ApiException` around the call as well, because a transport failure
still raises. Confirm on the `class ...` line before writing a catch block.
